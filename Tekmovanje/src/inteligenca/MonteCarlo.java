...
public class MonteCarlo extends Inteligenca{
	...
	public Poteza izberiPotezo(Igra igra) {
		...
		/* !!! Zavedam se, da trenutna implementacija časovne omejitve ni lepa. 
        ...
        */
		...
		while (vozlisce != null) { //dokler ne pridemo do debla
			...
			if (koncnoStanje == Stanje.NEODLOCENO) vozlisce.getStatus().posodobiZmage(NEODLOCENO);
				...
				if (vozlisce.getStatus().getGamePosition().naPotezi() == Igralec.WHITE) vozlisce.getStatus().posodobiZmage(ZMAGA);
			}
			else if (koncnoStanje == Stanje.ZMAGA_W) {
				if (vozlisce.getStatus().getGamePosition().naPotezi() == Igralec.BLACK) vozlisce.getStatus().posodobiZmage(ZMAGA);
			}
